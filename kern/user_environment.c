...
extern int pf_add_env_page(struct Env* ptr_env, uint32 virtual_address,
		void* ptrDataSrc);
extern int loadtime_map_frame(uint32 *ptr_page_directory,
...
struct UserProgramInfo userPrograms[] =
		{ { "fos_helloWorld", "Created by FOS team, fos@nowhere.com",
				PTR_START_OF(fos_helloWorld) }, { "fos_add",
				"Created by FOS team, fos@nowhere.com", PTR_START_OF(fos_add) },
				{ "fos_alloc", "Created by FOS team, fos@nowhere.com",
						PTR_START_OF(fos_alloc) }, { "fos_input",
						"Created by FOS team, fos@nowhere.com", PTR_START_OF(
								fos_input) }, { "fos_game",
						"Created by FOS team, fos@nowhere.com", PTR_START_OF(
								game) }, { "fos_static_data_section",
						"Created by FOS team, fos@nowhere.com", PTR_START_OF(
								fos_static_data_section) }, {
						"fos_data_on_stack",
						"Created by FOS team, fos@nowhere.com", PTR_START_OF(
								fos_data_on_stack) }, { "tqs",
						"Quicksort without freeHeap", PTR_START_OF(
								quicksort_heap) },

				/*TESTING 2016*/
				//[1] READY MADE TESTS
				{"tmod","test mod",PTR_START_OF(tst_mod_1)},
				{ "tpp", "Tests the Page placement", PTR_START_OF(tst_placement) },
				{ "tia", "tests handling of invalid memory access",
						PTR_START_OF(tst_invalid_access) },
				{ "tpr1",
						"Tests page replacement (allocation of Memory and PageFile)",
						PTR_START_OF(tst_page_replacement_alloc) }, { "tpr2",
						"Tests page replacement (clock algorithm)",
						PTR_START_OF(tst_page_replacement_clock) }, { "tpr3",
						"Tests page replacement (LRU algorithm)", PTR_START_OF(
								tst_page_replacement_lru) },
				{ "tpr4",
						"tests page replacement (handling new stack and modified pages)",
						PTR_START_OF(tst_page_replacement_stack) }, { "tm1",
						"tests malloc (1): start address & allocated frames",
						PTR_START_OF(tst_malloc_1) },
				{ "tm2",
						"tests malloc (2): writing & reading values in allocated spaces",
						PTR_START_OF(tst_malloc_2) },
				{ "tm3",
						"tests malloc (3): check memory allocation and WS after accessing",
						PTR_START_OF(tst_malloc_3) },

				{ "tf1",
						"tests free (1): freeing tables, WS and page file [placement case]",
						PTR_START_OF(tst_free_1) }, { "tf2",
						"tests free (2): try accessing values in freed spaces",
						PTR_START_OF(tst_free_2) },

				{ "tnf", "tests next fit: all cases", PTR_START_OF(tst_nextfit) },
				{ "tbf1", "tests best fit (1): always find suitable space",
						PTR_START_OF(tst_best_fit_1) }, { "tbf2",
						"tests best fit (2): no suitable space", PTR_START_OF(
								tst_best_fit_2) },

				{ "hp", "heap program (allocate and free from heap)",
						PTR_START_OF(heap_program) },

				//[2] PROGRAMS
				{ "tqsfh", "Quicksort with freeHeap", PTR_START_OF(
						tst_quicksort_freeHeap) }, { "qs1", "Quicksort v.1",
						PTR_START_OF(quicksort1) }, { "qs2", "Quicksort v.2",
						PTR_START_OF(quicksort2) }, { "qs3", "Quicksort v.3",
						PTR_START_OF(quicksort3) }, { "qs4", "Quicksort v.4",
						PTR_START_OF(quicksort4) }, { "qs5", "Quicksort v.5",
						PTR_START_OF(quicksort5) }, { "fact",
						"Factorial Recursive", PTR_START_OF(fos_factorial) }, {
						"fib", "Fibonacci Recursive", PTR_START_OF(
								fos_fibonacci) }, { "qs",
						"Quicksort with NO memory leakage", PTR_START_OF(
								quicksort_noleakage) }, { "ms1",
						"Mergesort with NO memory leakage", PTR_START_OF(
								mergesort_noleakage) }, { "ms2",
						"Mergesort that cause memory leakage", PTR_START_OF(
								mergesort_leakage) },

				//[3] BONUSES
				{ "tff1", "tests first fit (1): always find suitable space",
						PTR_START_OF(tst_first_fit_1) }, { "tff2",
						"tests first fit (2): no suitable space", PTR_START_OF(
								tst_first_fit_2) },
				{ "twf", "tests worst fit: all cases", PTR_START_OF(
						tst_worstfit) }, { "tr1",
						"tests realloc (1): normal cases", PTR_START_OF(
								tst_realloc_1) }, { "tr2",
						"tests realloc (2): special cases", PTR_START_OF(
								tst_realloc_2) }, { "tfr1",
						"tests freeRAM (1): run in specific scenario",
						PTR_START_OF(tst_freeRAM_1) }, { "tfr2",
						"tests freeRAM (2): run directly", PTR_START_OF(
								tst_freeRAM_2) }, { "tfifo1",
						"Tests page replacement (FIFO algorithm 1)",
						PTR_START_OF(tst_page_replacement_FIFO_1) }, { "tfifo2",
						"Tests page replacement (FIFO algorithm 2)",
						PTR_START_OF(tst_page_replacement_FIFO_2) }, {
						"tmodclk",
						"Tests page replacement (modified clock algorithm)",
						PTR_START_OF(tst_page_replacement_mod_clock) },
...
struct Env* env_create(char* user_program_name, unsigned int page_WS_size) {
	...
	PROGRAM_SEGMENT_FOREACH(seg, ptr_program_start) {
		...
		LOG_STATMENT(
				cprintf("SEGMENT: allocated pages in WS = %d", allocated_pages));
		LOG_STATMENT(
...
void env_run_cmd_prmpt() {
	panic(
			"env_run_cmd_prmpt() is obsoleted since 2015! use env_exit() instead");
	/*
